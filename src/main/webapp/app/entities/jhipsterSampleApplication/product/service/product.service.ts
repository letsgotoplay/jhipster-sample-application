...
type RestOf<T extends IProduct | NewProduct> = Omit<T, 'liveDate'> & {
...
export class ProductService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/products', 'jhipstersampleapplication');
  ...
  protected convertDateFromClient<T extends IProduct | NewProduct | PartialUpdateProduct>(product: T): RestOf<T> {
    return {
      ...product,
      liveDate: product.liveDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProduct: RestProduct): IProduct {
    return {
      ...restProduct,
      liveDate: restProduct.liveDate ? dayjs(restProduct.liveDate) : undefined,
    };
  }
